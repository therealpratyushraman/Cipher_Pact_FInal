...
def decrypt_and_decompress(input_file, output_file, passphrase):
    ...
    print("Decryption and Decompression Complete.")
...
def compress_video(input_file, output_file):
    try:
        subprocess.run([
            ...
            '-i', input_file,
            '-vf', 'scale=1280:720',
            '-b:v', '500k',  # Lower video bitrate for better compression
            '-c:v', 'libx264',
            '-crf', '28',  # Higher CRF for better compression
            '-preset', 'medium',  # Adjust the preset for speed vs compression ratio
            '-c:a', 'aac',
            '-b:a', '128k',
            output_file
        ], check=True)
